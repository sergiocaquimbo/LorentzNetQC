...
from quantumCircuits import DressedQuantumCircuit
...
class LEQB(nn.Module):
    def __init__(self, n_input, n_output, n_hidden, n_node_attr=0,
        ...
        # in_features = 2 * n_input + 2  # original
        # self.pre_message = nn.Linear(in_features, 10) # extra
        ...
        """
        self.phi_e = DressedQuantumCircuit(n_input)
...
#             nn.Linear(n_hidden, n_output))
        ...
        self.phi_h = DressedQuantumCircuit(n_hidden)
...
#             layer)
        ...
        self.phi_x = nn.Sequential(
            DressedQuantumCircuit(n_hidden),
            layer)
...
#             nn.Sigmoid())
    ...
    def m_model(self, hi, hj, norms, dots):
        ...
        # out = self.pre_message(out) # extra
...
class LorentzEQGNN(nn.Module):
    ...
    def __init__(self, n_scalar, n_hidden, n_class = 2, n_layers = 6, c_weight = 1e-3, dropout = 0., A=None, include_x=False):
        super(LorentzEQGNN, self).__init__()
